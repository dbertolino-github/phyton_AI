...
and familiarize with some of the data we'll be reused in next excercises.
'''
...
#FUNCTION TO EXTRACT DATA FROM tar.gz COMPRESSED FILE
...
'''
We want to represent our images as multi-dimensional arrays (img index, x, y) 
so that is possible to store them in memory.
Tensorflow libraries use tensor, a simple n-dimensional array to achieve this purpose.
In order to make things simpler, we save tensor arrays in pickle file that can be reused.
'''
